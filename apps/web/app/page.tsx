            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[720px] text-left text-sm">
                ...
                <tbody className="divide-y divide-[#e8ebf0]">
                  {jobs.slice(0, 5).map((job) => <tr key={job.id} className="hover:bg-[#fafbfd]"><td className="utility-face px-5 py-4 text-xs text-[#667085]">{job.id.slice(0, 12)}</td><td className="px-5 py-4 font-bold capitalize">{job.platform?.replace("_", " ") ?? "图片处理"}</td><td className="px-5 py-4">{job.image_slot ?? job.task_type}</td><td className="px-5 py-4"><Badge className={cn(job.status === "completed" && "border-emerald-200 bg-emerald-50 text-emerald-700", job.status === "processing" && "border-orange-200 bg-orange-50 text-orange-700", job.status === "pending" && "border-amber-200 bg-amber-50 text-amber-700")}>{job.status}</Badge></td><td className="px-5 py-4 text-[#7b8698]">{new Date(job.created_at).toLocaleString("zh-CN", {hour12: false})}</td></tr>)}
                </tbody>
              </table>
            </div>
          </Card>
